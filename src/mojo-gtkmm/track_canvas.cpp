...
TrackCanvas::add (Glib::RefPtr<TrackCanvasItem> item)
{
	...
	item->signal_height_changed ().connect
		(
		 sigc::bind
		 (
...
double
TrackCanvas::get_y_position_for_new_track ()
{
	double new_y_pos = 0;

	for (tci_list_t::const_iterator i = m_track_canvas_items.begin ();
			i != m_track_canvas_items.end (); ++i)
	{
...
void
TrackCanvas::on_track_canvas_item_height_changed (Glib::RefPtr<TrackCanvasItem> ptr)
{
	LOG;
