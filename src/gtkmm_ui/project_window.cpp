...
{
	m_builder =
	    Gtk::Builder::create_from_file(get_ui_filepath("project_window.ui"));
...
void ProjectWindow::connect_file_menu_actions()
{
	connect_action(
	    m_builder, "project-new-action", sigc::ptr_fun(App::new_project));

	connect_action(
	    m_builder, "project-open-action", sigc::ptr_fun(App::open_project));

	connect_action(m_builder,
	               "project-save-action",
	...
	connect_action(
	    m_builder, "import-audio-action", sigc::ptr_fun(&App::open_import_dialog));

	connect_action(m_builder,
	               "project-close-action",
	               sigc::bind(sigc::ptr_fun(&App::close_project), m_project));

	connect_action(m_builder, "app-quit-action", sigc::ptr_fun(App::quit));
}

void ProjectWindow::connect_project_menu_actions()
{
	connect_action(m_builder,
	               "audio-track-add-action",
...
void ProjectWindow::connect_view_menu_actions()
{
	connect_toggleaction(
	    m_builder,
	    "project-window-fullscreen-toggleaction",
	    sigc::mem_fun(this, &ProjectWindow::on_fullscreen_toggled));
}

void ProjectWindow::pack_children()
{
	Gtk::VBox* vbox1 = 0;
...
void ProjectWindow::on_fullscreen_toggled()
{
	ToggleActionGPtr action =
	    get_toggleaction(m_builder, "project-window-fullscreen-toggleaction");
