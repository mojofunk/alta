...
{
	App::get_dispatcher().call_async(
	    sigc::bind(sigc::mem_fun(this, &TrackView::on_track_added), track));
}

void TrackView::on_track_removed_handler(mojo::Track* track)
{
	App::get_dispatcher().call_sync(
	    sigc::bind(sigc::mem_fun(this, &TrackView::on_track_removed), track));
}

void TrackView::on_track_added(mojo::Track* track)
{
