...
public:
	...
	pointer get() const { return g_atomic_pointer_get(&m_ptr); }

	void set(pointer new_ptr) { g_atomic_pointer_set(&m_ptr, new_ptr); }

	bool compare_and_exchange(pointer old_ptr, pointer new_ptr)
	{
		return g_atomic_pointer_compare_and_exchange(&m_ptr, old_ptr, new_ptr);
	}
