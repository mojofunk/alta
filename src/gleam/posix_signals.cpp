...
TraceMode
get_trace_mode() { return stack_trace_mode; }

void
set_trace_mode(TraceMode mode) { stack_trace_mode = mode; }
...
void
termination_signal_handler (int sig_num)
{
	switch (sig_num)
	{
		case SIGHUP:
...
void
fatal_signal_handler (int sig_num)
{
	// handle SIGILL ?
	switch (sig_num)
	{
