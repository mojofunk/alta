...
std::string
PortmidiMIDIOutputDevice::get_name() const
{
	return get_device_info()->name;
}

MIDIDevice::error_t
PortmidiMIDIOutputDevice::open(time_callback_t* callback, void* user_data)
{
	int latency = 0;
...
MIDIDevice::error_t
PortmidiMIDIOutputDevice::close()
{
...
std::string
PortmidiMIDIOutputDevice::get_error_string(error_t err)
{
	return Pm_GetErrorText((PmError)err);
}

MIDIDevice::error_t
PortmidiMIDIOutputDevice::write(Event* buffer, int32_t size)
{
...
PmDeviceInfo const*
PortmidiMIDIOutputDevice::get_device_info() const
{
