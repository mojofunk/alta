 ...
 *
 * should external classes have direct access to Node controls or should all parameter
 ...
 *
 * Each Node can have zero or more input and output ports. A Node doesn't have to
 * have same number of input ports. For instance a Node could take a stereo input
 * using two inputs and have 6 output ports for 5.1 surround sound.
 *
 * A Node might also have different types of ports. for instance a midi input port
 * and an audio output port in the case of an Midi Instrument track  
 *
 * To make the Engine versatile and ignorant of Node implementations I think Nodes
 ...
 * responsible for syncronizing thread access to buffers during the process cycle
 *
